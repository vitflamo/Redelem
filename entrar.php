...
if (isset($_POST[entra])){
    ...
    if (db::query('SELECT conta_apelido FROM conta WHERE conta_apelido=:apelido', array(':apelido'=>$apelido))){
        ...
        if(password_verify($senha, db::query('SELECT conta_senha FROM conta WHERE conta_apelido=:apelido',
        array(':apelido'=>$apelido))[0]['conta_senha'])){
            ...
            $conta_id = db::query('SELECT conta_id FROM conta WHERE conta_apelido=:apelido', 
            array(':apelido'=>$apelido))[0]['conta_id'];
            ...
            setcookie("IDRS_TEMP", '1', time() + 60 * 60 * 24 * 3, '/', null, null, true);
